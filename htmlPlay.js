(function(){
    ...
    // Get random initial positions for the browser window
    var randomX = getRandomPosition(-2, 2);
    var randomY = getRandomPosition(-2, 2);

    var css = ".browzr{z-index:99999;position:fixed;left:" + randomX + "px;top:" + randomY + "px;background-color:#f5f5f5;border:2px solid #ccc;border-radius:5px;padding:20px;box-shadow:0 0 10px rgba(0,0,0,0.5);}"+
    ...
    div.innerHTML = "<input type='text' id='htmlenter' placeholder='Enter HTML here...'><button class='button' onclick='dothething();'>Preview</button><button class='button' onclick='downloadContent();'>Download</button><button class='minimize' onclick='minimize();'>-</button><br><iframe id='htmlframe'></iframe>";
    document.body.appendChild(div);

    // Create the tab for unminimizing
    ...
    window.dothething = function() {
        ...
        document.getElementById('htmlframe').srcdoc = htmlContent;
    };
